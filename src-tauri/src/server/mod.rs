...
use crate::{LANDMARKS, APP_HANDLE};
...
async fn landmarks(_req: Request<Body>) -> Result<Response<Body>, Infallible> {
  ...
  if landmarks.is_none() {
    ...
    return Ok(Response::new(serde_json::to_string(&response).unwrap().into()));
  }
  ...
  Ok(Response::new(serde_json::to_string(&response).unwrap().into()))
}
...
async fn handle_req(_req: Request<Body>) -> Result<Response<Body>, Infallible> {
  ...
  if _req.uri().path() == "/api/getexercise" {
...
#[tokio::main]
pub async fn spawn_tokio(port: u16, rx: Receiver<()>) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
